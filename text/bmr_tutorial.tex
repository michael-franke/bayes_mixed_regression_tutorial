...
\colorbox{mygray}{\centering
  \begin{minipage}{1\textwidth}
    ...
    For example, lines 6--7 of the summary of \texttt{modelFE} tell us that we collected a total of 4000 samples from the posterior distribution. These came from four chains, each running for 2000 iterations, but discarding the first 1000 samples as \textbf{warm up}, since the initial starting point might be quite ``unrepresentative''. The reported estimate of \textbf{efficient samples} is important, too. It gives an estimate of the number of samples, for each parameter, which appear to be sufficiently independent of their prior history (e.g., not the outcome of being stuck for a long time in an ``unrepresentative area'').
