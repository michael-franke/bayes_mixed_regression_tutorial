...
%
\marginnote[-2cm]{Supplying a \textbf{prior}, really just \textit{any} prior, is important to get a
...
%
\marginnote{More on the use of the \texttt{faintr} package can be found here: \url{https://michael-franke.github.io/bayes_mixed_regression_tutorial/faintr/doc/faintr_basics.html}.}
%
...
We then calculate the probability for our hypotheses again. \marginnote{The argument `control' here allows us to tweak the sampling procedure in order to ensure convergence. \texttt{brms} will encourage you to do so whenever you run into such convergence issues.}
...
\marginnote{This section is a kind of excursion into more foundational matters. You
  \emph{could} possibly skip it, but we \emph{strongly recommend} that you read it at some
  point, and possibly even follow up on some of the references if you want to pursue the
  Bayesian road further.}
...
%
\marginnote{An apparent exception is
the use of model comparison in hierarchical regression models, for which the calculation of
$p$-values is, let's say, difficult.} 
%
...
%
% A model $M_i$ comprises a prior $P(\theta \mid M_i)$ over parameter vectors $\theta$ and a likelihood function $P(D
% \mid \theta, M_i)$.
%
...
analyses.\marginnote{A. Solomon Kurz has an online book based on
