...
\colorbox{mygray}{\centering
  \begin{minipage}{1.5\textwidth}
    ...
    some sanity checking. That's why it is common to inspect your samples, visually by
    plotting, and by certain numeric measure indicating how ``good'' your samples really are.
    ...
    The problem is that we can be sure that the visits are ``good'' samples from the posterior
    distribution only if we take infinitely many. Usually, Franz is too lazy to jump that much
    (and we are too busy). If we only have a small-ish number of samples, it might be that
    Franz was sitting (with his fat \dots) on the same lily pad for a looooong time. While he
    sat there, we did not learn much about the posterior's shape. Variants of basic MCMC, like
    Hamiltonian Monte Carlo which \texttt{brms} uses, cleverly circumnavigate this problem to a
    wide extent. Still, we should not trust Franz alone. So we routinely run several
    \textbf{chains} (per defaul \texttt{brm} uses 4 chains). Think of this as letting go Franz,
    ...
    where they sat, because they might have started at some far-off place with hardly any flies
    at all, so that the first jumps are really not interesting. This is the so-called
    ...
    history of froggy jumps are similar enough; techinically: the \textbf{chains have
      converged}. of the froggies commonly to check if the samples are good enough, we can
    investigate. The number of \textbf{efficient samples} is an estimate of, roughly put, the
    number of informative jumps that out froggies made, when we subtract periods where they
